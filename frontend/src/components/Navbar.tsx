...
export default function Navbar() {
    ...
    return (
        <>
            {/* ── Main Navbar ── */}
            <nav className={`${isHome ? "absolute top-0 left-0 right-0 z-50 bg-transparent" : "sticky top-0 z-50 bg-white border-b border-gray-200 shadow-sm"}`}>
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="flex justify-between items-center h-14">
                        {/* Logo */}
                        <Link href="/" className="flex items-center gap-2" onClick={closeMenu}>
                            <BookOpen size={22} className={isHome ? "text-white" : "text-indigo-600"} />
                            <span className={`font-bold text-lg tracking-tight ${isHome ? "text-white" : "text-gray-900"}`}>BookBarter</span>
                        </Link>

                        {/* Desktop Nav — centered */}
                        <div className="hidden md:flex absolute left-1/2 -translate-x-1/2 items-center gap-8">
                            <Link href="/how-it-works" className={`text-sm font-semibold transition-colors ${isHome ? "text-gray-300 hover:text-white" : "text-gray-600 hover:text-gray-900"}`}>
                                How it Works
                            </Link>
                            <Link href="/marketplace" className={`text-sm font-semibold transition-colors ${isHome ? "text-gray-300 hover:text-white" : "text-gray-600 hover:text-gray-900"}`}>
                                Browse Books
                            </Link>
                        </div>
                        ...
                        <div className="hidden md:flex items-center gap-4 ml-auto">
                            {mounted && user ? (
                                <div className="flex items-center gap-5">
                                    <Link href="/dashboard/wallet" className={`flex items-center gap-1.5 font-medium transition-colors text-sm ${isHome ? "text-gray-300 hover:text-yellow-400" : "text-gray-600 hover:text-yellow-500"}`}>
                                        <Wallet size={16} className="text-yellow-500" />
                                        <span>{user.coins} Coins</span>
                                    </Link>
                                    <Link href="/dashboard/books/add" className={`flex items-center gap-1.5 text-sm text-white font-semibold px-3 py-1.5 rounded-lg transition-colors ${isHome ? "bg-orange-500 hover:bg-orange-600" : "bg-indigo-600 hover:bg-indigo-700"}`}>
                                        <BookOpen size={15} /> List Book
                                    </Link>
                                    <div className="relative group">
                                        <button className={`flex items-center gap-2 transition-colors ${isHome ? "text-gray-300 hover:text-white" : "text-gray-600 hover:text-gray-900"}`}>
                                            <UserCircle size={26} />
                                ...
                                <>
                                    <Link href="/login" className={`text-sm font-semibold transition-colors ${isHome ? "text-gray-300 hover:text-white" : "text-gray-600 hover:text-gray-900"}`}>Login</Link>
                                    <Link href="/register" className={`text-sm text-white font-semibold px-4 py-1.5 rounded-lg transition-colors ${isHome ? "bg-orange-500 hover:bg-orange-600" : "bg-indigo-600 hover:bg-indigo-700"}`}>Sign Up</Link>
                                </>
                            ) : null}
                        </div>
                        ...
                        <div className="md:hidden flex items-center">
                            <button
                                ...
                                className={`focus:outline-none p-2 -mr-2 rounded-lg transition-colors ${isHome ? "text-white hover:text-gray-300" : "text-gray-600 hover:text-gray-900"}`}
